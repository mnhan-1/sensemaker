(function(){
  ...
  // Served, the fetch path wins: a pod that edits content/*.md in its repo sees
  // the edit without regenerating the shim. Opened by double-click, fetch on
  // file:// is blocked by the browser — so don't try, and don't spend four red
  // console errors proving it. The shim is always shipped.
  if(location.protocol==='file:'&&window.SITE_DATA){render(window.SITE_DATA);return;}
